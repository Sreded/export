(function () {
  ...
  function start() {
    ...
    document.addEventListener("keydown", function (event) { if (event.key === "Escape" && !els.detailOverlay.hidden) closeDetail(); });
    setupDragAndDrop();
  }
  ...
  function normalizeEntry(row, fields) {
    ...
    const text = cleanValue(fields.text ? row[fields.text] : "");
    const title = cleanValue(fields.title ? row[fields.title] : "");
    const metadata = {};
  ...
  function parseFlexibleDate(value) {
    ...
    if (typeof value === "number" || /^-?\d+(?:\.\d+)?$/.test(raw.trim())) {
      ...
      if (Number.isFinite(number)) {
        candidates.push(number > 1e11 ? number : number > 1e8 ? number * 1000 : number);
